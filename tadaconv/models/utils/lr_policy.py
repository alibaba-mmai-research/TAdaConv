...
def lr_func_cosine_v2(cfg, cur_epoch):
    """
    Retrieve the learning rate to specified values at specified epoch with the
    cosine learning rate schedule. Details can be found in:
    Ilya Loshchilov, and  Frank Hutter
    ...
    Args:
        ...
        cur_epoch (float): the number of epoch of the current training stage.
    """
    offset = cfg.OPTIMIZER.WARMUP_EPOCHS if cfg.OPTIMIZER.COSINE_AFTER_WARMUP else 0.0
    assert cfg.OPTIMIZER.COSINE_END_LR < cfg.OPTIMIZER.BASE_LR
    return (
        cfg.OPTIMIZER.COSINE_END_LR
        + (cfg.OPTIMIZER.BASE_LR - cfg.OPTIMIZER.COSINE_END_LR)
        * (
            math.cos(
                math.pi * (cur_epoch - offset) / (cfg.OPTIMIZER.MAX_EPOCH - offset)
            )
            + 1.0
        )
        * 0.5
