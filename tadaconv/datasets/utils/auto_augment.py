...
class AugmentOp:
    ...
    def __call__(self, img):
        ...
        return [self.aug_fn(i, *level_args, **self.kwargs) for i in img]
...
class RandAugment:
    ...
    def __call__(self, img):
        ...
        img_numpy = (img.permute(1, 2, 3, 0)*255.0).numpy().astype(np.uint8)
        img_list = [Image.fromarray(img_numpy[i]) for i in range(img_numpy.shape[0])]
        ops = np.random.choice(
            self.ops, self.num_layers, replace=self.choice_weights is None, p=self.choice_weights)
        for op in ops:
            img_list = op(img_list)
        img = torch.from_numpy(np.stack([np.asarray(img_list[i]).astype(np.float32) for i in range(len(img_list))], axis=0))
        img = img.permute(3, 0, 1, 2)/255.0
        return img
...
def creat_auto_augmentation(aug_str, img_size, mean):
    ...
    return rand_augment_transform(aug_str, aa_params)
